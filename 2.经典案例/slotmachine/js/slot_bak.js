...
(function(){
	var Slot = function(el, pics){
		if(this instanceof Slot){
			...
			this.reTemp = undefined;
			this.ended = false;
			this.frame = this.maxSpeed/this.down;
			this.distance = this.down * this.frame * this.frame / 2 + 75;
			this.hit = this.distance % (this.height * 3) - this.height;
			this.reels = [
				{
					v: 0,slowing: false,complete:false,grids: [{index :0,pos: {x:0,y:0}},{index: 1,pos: {x:0,y:this.height}},{index: 2,pos: {x:0,y:2*this.height}}]
				},
				{
					v: 0,slowing: false,complete:false,grids: [{index :1,pos: {x:this.reel_w,y:0}},{index: 2,pos: {x:this.reel_w,y:this.height}},{index: 0,pos: {x:this.reel_w,y:2*this.height}}]
				},
				{
					v: 0,slowing: false,complete:false,grids: [{index :2,pos: {x:2*this.reel_w,y:0}},{index: 0,pos: {x:2*this.reel_w,y:this.height}},{index: 1,pos: {x:2*this.reel_w,y:2*this.height}}]
				}
			];
			this._init();
		}else{
	...
	Slot.prototype = {
		_init: function(){
			...
			for(var i=0,len=_this.pics.length;i<len;i++){
				(function(i){
					var image = new Image();
					image.src = _this.pics[i];
					image.onload = function(){
						_this.images.push(image);
						_this.images.length === 3 && _this.render();
					};
				})(i);
			}
		},
